...
class PipelineWorker:
    ...
    def _check_version_consistency(self, session: Session, run: Run) -> None:
        """Check that all schema versions used in the run are consistent.
        
        This method collects schema version information from the run's proposal,
        persona reviews, and decision, then verifies that all schemas are using
        consistent versions within this deployment.
        
        Args:
            session: Database session
            run: Run instance with completed proposal, reviews, and decision
            
        Raises:
            SchemaValidationError: If version inconsistency is detected (logged, not fatal)
        """
        schema_versions: list[dict[str, Any]] = []

        # Collect schema versions from proposal
        if run.proposal_version and run.proposal_version.expanded_proposal_json:
            ...
            if "_schema_version" in proposal_json:
        ...
        # Collect schema version from decision
        if run.decision and run.decision.decision_json:
            if "_schema_version" in run.decision.decision_json:
                schema_versions.append({
        ...
        if schema_versions:
            try:
                check_version_consistency(
                    ...
                    context={"run_id": str(run.id), "run_type": run.run_type.value},
                )
                logger.info(
                ...
                # Re-raise to be caught and logged by caller
                raise
    ...
    def _process_job(self, session: Session, job_msg: JobMessage) -> None:
        ...
        try:
            ...
            # Check version consistency across all outputs
            try:
                self._check_version_consistency(session, run)
                ...
                # Don't fail the run, but log the warning
                # This allows the system to continue operating even with version inconsistencies
